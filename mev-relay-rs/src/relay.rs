...
use mev_build_rs::{
    sign_builder_message, verify_signed_consensus_message,
    ...
    ValidatorSummaryProviderError, ValidatorSummaryProvider, ValidatorRegistrar, ValidatorRegistrationStatus
};
...
pub enum Error {
    ...
    #[error("{0}")]
    Validator(#[from] ValidatorSummaryProviderError),
}
...
impl BlindedBlockProvider for Relay {
    async fn register_validators(
        ...
        let mut new_registrations = {
            let mut state = self.state.lock();
            let registrar = ValidatorRegistrar::new(&self.validators, &self.context);
            ...
            for registration in registrations.iter_mut() {
                ...
                let status = registrar.validate_registration(
                    registration,
                    current_time,
                    latest_timestamp,
                )?;
