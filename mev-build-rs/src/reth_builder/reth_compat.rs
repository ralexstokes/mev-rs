...
pub(crate) fn to_execution_payload<Pool>(
    value: &SealedBlock,
    pool: &Pool,
    fork: &str,
) -> ExecutionPayload
    ...
    Pool: TransactionPool,
{
    ...
    match fork {
        "bellatrix" => {
            ...
            ExecutionPayload::Bellatrix(bellatrix_payload)
        }
        "capella" => {
            ...
            ExecutionPayload::Capella(capella_payload)
        }
        "deneb" => {
            let withdrawals = withdrawals
                .as_ref()
            ...
            let blob_tx_hashes =
                value.blob_transactions().iter().map(|t| t.hash()).collect::<Vec<_>>();
            let blobs_bundle = get_blob_bundles(pool, blob_tx_hashes);
            let deneb_payload_bundles = deneb::ExecutionPayloadAndBlobsBundle {
                execution_payload: deneb_payload,
                blobs_bundle,
            };
